...
from ch5_neural_network.bp import StandardBP, AdaptedBP
...
if __name__ == '__main__':
    ...
    adapted_bp = AdaptedBP()
    adapted_bp.init(input=feature_n, hidden=10, ouput=y.shape[1])

    st = time.time()
    accum_loss = adapted_bp.train(X=X, y=y, eta=0.01, n=max_iter)
    ed = time.time()
    probability = adapted_bp.predict(X)
    y_predict = [1 if _p[0] > 0.5 else 0 for _p in probability]
    error = 0
    for _index, _y in enumerate(y.reshape(sample_n)):
        if _y != y_predict[_index]:
            error += 1
    print(f'自适应BP网络正确率：{round(1-error/len(y), 2)}, 用时：{round(ed-st, 2)}s')

    ##Loss可视化
    plt.figure()
    plt.plot([i+1 for i in range(max_iter)], standard_loss)
    plt.plot([i+1 for i in range(max_iter)], accum_loss)
    plt.legend(['standard BP', 'self-adapted BP'])
